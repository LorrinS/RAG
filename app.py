    ...
    try:
        with open(file_path, "rb") as f:  # Open the file in binary mode
            ...
            for page in reader.pages:
                text += page.extract_text() or ""  # If the page has no text, return empty string
            return text
    except Exception as e:
...
def list_local_files():
    ...
    for file_name in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path) and file_name.endswith(".pdf"):  # Only process .pdf files
...
# Add the UserSession class here

class UserSession:
    def __init__(self):
        # Initialize the session's data
        self.context = ""  # Store session context (history of queries, etc.)
        self.query_history = []  # Store the user's previous queries
        self.responses_history = []  # Store the responses for each query

    def update_context(self, query: str, response: str):
        """
        Update the session context with the new query and response.
        """
        self.query_history.append(query)
        self.responses_history.append(response)
        # Append query and response to context for future references
        self.context += f"\nUser query: {query}\nAnswer: {response}\n"

    def get_context(self) -> str:
        """
        Return the current context of the session, which includes the history of queries and responses.
        """
        return self.context
